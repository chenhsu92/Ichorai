...
function CareersPage({ navigate }) {
  ...
  const roles = [
    ...
    { id:2, title:"Full-Stack Engineer (React / Python)", type:"Full-Time", loc:"London, UK (Hybrid)", sal:"Competitive + Equity",
      desc:"[ Placeholder — edit with your full job description. Example: Building the Graphical Interface and core platform infrastructure. You will own the product layer that allows domain experts to tune AI models without engineering support. Strong React and Python required. ]",
  ...
  return (
    <>
      ...
      {/* Values */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", borderBottom:"1px solid var(--g800)" }}>
        {[
          ...
          <div key={i} style={{ padding:"32px 26px", borderRight:i<3?"1px solid var(--g800)":"none" }}>
            <div style={{ fontSize:"26px", color:"var(--g600)", marginBottom:"14px" }}>{v.icon}</div>
      ...
      <section style={{ padding:"36px 48px 72px" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:"2px" }}>
          {roles.map((role,i) => (
            <div key={role.id} style={{ border:"1px solid var(--g600)", overflow:"hidden", transition:"border-color 0.22s" }}
              onMouseEnter={e => e.currentTarget.style.borderColor="var(--green)"}
              onMouseLeave={e => e.currentTarget.style.borderColor="var(--g600)"}
            >
              ...
              {selected===role.id && (
                <div style={{ padding:"0 32px 28px", animation:"fadeUp 0.3s ease", borderTop:"1px solid var(--g800)" }}>
        ...
        {/* Speculative */}
        <div style={{ marginTop:"40px", border:"1px dashed var(--g600)", padding:"36px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"20px" }}>
          <div>
